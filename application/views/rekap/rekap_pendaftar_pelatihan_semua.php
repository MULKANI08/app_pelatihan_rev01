...
<h4><p align="center">LAPORAN DATA PENDAFTAR PELATIHAN</p></h4>
<h4><p>Di Cetak Hari
<?php
 $hari   = date('l');
...
$query = $this->db->query("SELECT * FROM peserta, pendaftar where peserta.status_pelatihan = 'Belum Diverifikasi' AND peserta.id_pendaftar = pendaftar.id_pendaftar AND pendaftar.jenis_kelamin = 'Laki-Laki'");
    		if($query->num_rows() > 0){
				echo $query->num_rows();
			}else{
				echo 0;
...
$query = $this->db->query("SELECT * FROM peserta, pendaftar where peserta.status_pelatihan = 'Belum Diverifikasi' AND peserta.id_pendaftar = pendaftar.id_pendaftar AND pendaftar.jenis_kelamin = 'Perempuan'");
    		if($query->num_rows() > 0){
				echo $query->num_rows();
			}else{
				echo 0;
