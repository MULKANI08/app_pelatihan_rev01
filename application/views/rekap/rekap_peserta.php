...
<h4><p style="text-transform: uppercase;" align="center">LAPORAN DATA PESERTA PELATIHAN BERDASARKAN NAMA PELATIHAN <br> <?php echo $_POST['filter']; ?></p></h4>
<h4><p>Di Cetak Hari
<?php
 $hari   = date('l');
...
$query = $this->db->query("SELECT * FROM peserta, pendaftar where peserta.status_pelatihan = 'Diterima' AND peserta.id_pendaftar = pendaftar.id_pendaftar AND peserta.tanggal_daftar BETWEEN '$tanggal1' AND '$tanggal2' AND pendaftar.jenis_kelamin = 'Laki-Laki'");
    		if($query->num_rows() > 0){
				echo $query->num_rows();
			}else{
				echo 0;
...
$tanggal1=$this->input->post('tanggal1');
$tanggal2=$this->input->post('tanggal2');
$query = $this->db->query("SELECT * FROM peserta, pendaftar where peserta.status_pelatihan = 'Diterima' AND peserta.id_pendaftar = pendaftar.id_pendaftar AND peserta.tanggal_daftar BETWEEN '$tanggal1' AND '$tanggal2' AND pendaftar.jenis_kelamin = 'Perempuan'");
    		if($query->num_rows() > 0){
				echo $query->num_rows();
			}else{
				echo 0;
